...
from utils.plot import figure
from utils.plot import cmap
from utils.plot import path

from utils.plot.figure import np,pd,mpl,plt,sns,Path
from utils.general import rng,Block,Iterable
from utils.general import handle_type_to_list,update_dict
import utils.df as ut_df
...
from utils.plot.figure import rc_mpl,rc_default,rc_blank
from utils.plot.figure import rc_frame,rc_frame_lt,rc_box
from utils.plot.figure import rc_tl_notick,rc_tl_off_x,rc_tl_off_y,rc_tl_xt,rc_tl_yr
...
from utils.plot.figure import tl_rc,tl_fontdict,tl_savefig
...
|函数||参数检验|简要说明|
|:-|:-|:-|:-|
|[scipy.stats.ttest_1samp](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_1samp.html#scipy.stats.ttest_1samp)|`X=popmean`||t检验(n>=30)|
|[scipy.stats.ttest_1samp]((https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_1samp.html#scipy.stats.ttest_1samp))(X-Y,0)|`X-Y=0`||配对t检验|
|[scipy.stats.ttest_ind](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_ind.html#scipy.stats.ttest_ind)|`X=Y`||独立样本t检验|
...
|[scipy.stats.mannwhitneyu](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.mannwhitneyu.html#scipy.stats.mannwhitneyu)||非参数检验|**U检验**,用于检验分布之间位置的差异|""",
    raw=True)
...
def tl_format_stats_res(res,tag='statistic',precision=4):
    return "{tag} = {statistic}\npvalue = {pvalue}\n{significance_marker}".format(
        tag=tag,statistic=round(res.statistic,precision),
           pvalue=round(res.pvalue,precision),
...
def tl_GaussianKDE_evaluate(X,X_evaluate=None,bw_method='scott',return_X_evaluate = False):
    X_evaluate = np.arange(X.min(),X.max(),(X.max()-X.min())/50)
...
"""
    ...
    with Block("box",context=dict_box) as context:
        ...
        for i,row in df_para.iterrows():
            context.lines_func(row['positions'],row['q4_1'],row['q4_3'],color = context.lines_color,lw=context.lines_lw1,
                              zorder=context.zorder)
            context.lines_func(row['positions'],row['max'],row['q4_3'],color = context.lines_color,lw=context.lines_lw2,
                              zorder=context.zorder)
            context.lines_func(row['positions'],row['min'],row['q4_1'],color = context.lines_color,lw=context.lines_lw2,
                              zorder=context.zorder)
...
def bubble(df_size, ax, df_color='pink',fontdict_xtick=None,fontdict_ytick=None, **kwargs):
    """气泡图
